...
def _extract_data_from_image(pil_image, upload_folder, page_num=1):
    ...
    current_app.logger.info(f"--- Teks Halaman {page_num} ---\n{full_text_for_logging}\n--------------------")
    ...
    for pattern in rek_patterns:
        ...
        if match:
            ...
            current_app.logger.info(f"Halaman {page_num}: Kode Setor ditemukan: {kode_setor}")
            break
    ...
    for text in all_text_blocks:
        ...
        if match_fuzzy:
            ...
            if best_match:
                try: tanggal_obj = datetime(int(year), all_months[best_match], int(day)).date(); break
                except ValueError: pass
        match_slash = date_pattern_slash.search(text)
        if match_slash:
            try: day, month, year = map(int, match_slash.groups()); tanggal_obj = datetime(year, month, day).date(); break
            except ValueError: pass
    if tanggal_obj: current_app.logger.info(f"Halaman {page_num}: Tanggal ditemukan: {tanggal_obj}")

    # Logika Parsing JUMLAH
    candidate_values = []
    ...
    for text in all_text_blocks:
        if any(key in text for key in keywords):
            for num_str in money_pattern.findall(text):
                ...
                if value and len(str(value)) < 15: candidate_values.append(value)
    if not candidate_values:
        for text in all_text_blocks:
            for num_str in money_pattern.findall(text):
                ...
                if value and len(str(value)) < 15: candidate_values.append(value)
    if candidate_values:
        valid_candidates = [v for v in candidate_values if v > 1000]
        jumlah = max(valid_candidates if valid_candidates else candidate_values)
        current_app.logger.info(f"Halaman {page_num}: Jumlah final dipilih: {jumlah}")

    return {"kode_setor": kode_setor, "jumlah": jumlah, "tanggal": tanggal_obj.isoformat() if tanggal_obj else None, "preview_filename": preview_filename}
